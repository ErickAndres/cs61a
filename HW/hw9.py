...
def read_tail(tokens):
    ...
    elif tokens[0] in LEFT_RIGHT.values():
        #code here
        tokens.pop(0)
        return nil
...
def brack_read(tokens):
    ...
    if n != None:
        ...
        right_bracket = LEFT_RIGHT[token]
        #if len(right_bracket) > 1:
            #raise SyntaxError('unexpected ' + str(token))
        operation = BRACKETS[(token, right_bracket)]
        operands = read_tail(tokens)
        if len(right_bracket) > 1:
            raise SyntaxError('unexpected ' + str(token))
        return Pair(operation, operands)
    else:
        raise SyntaxError('unexpected ' + str(token))
    #answer to this part after right_bracket = LEFT_RIGHT[token]    
        #rest = read_tail(tokens)
        #right = tokens.pop(0)
