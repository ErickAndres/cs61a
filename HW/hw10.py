...
class Mobile:
    ...
    def is_balanced(self):
        ...
        return self.left.contents.is_balanced() and self.right.contents.is_balanced() and self.left.torque == self.right.torque
...
def interpret_mobile(s):
    ...
    for t in ('Branch', 'Weight', 'Mobile'):
        "*** YOUR CODE HERE ***"
        substitute_1 = s[0:next_T] + t + s[next_T + 1:]
        substitute = interpret_mobile(substitute_1)
        if type(substitute) is Mobile:
            return substitute
    return None
...
def scale_stream(s, k):
    ...
    """
    "*** YOUR CODE HERE ***"
    ints = Stream(s.first * k, lambda: scale_stream(s.rest, k))    
    return ints
...
def merge(s0, s1):
    ...
    if e0 < e1: 
        ...
        no_repeats = Stream(e0, compute_rest)  
        return no_repeats            
    elif e0 > e1:
        def compute_rest():
            return merge(s0, s1.rest)
        no_repeats = Stream(e1, compute_rest)
        return no_repeats
    else:
        def compute_rest():
...
def make_s():
    ...
    """
    def rest():
        first_merge =  merge(scale_stream(make_s(), 2), scale_stream(make_s(), 3))
        second_merge = scale_stream(make_s(), 5)
