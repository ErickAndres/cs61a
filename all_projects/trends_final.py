...
def make_tweet_fn(text, time, lat, lon):
    ...
    38
    """
    def dispatch(tweet_value):
        if tweet_value == 'text':
            return text
        elif tweet_value == 'time':
            return time
        elif tweet_value == 'lat':
            return lat
        elif tweet_value == 'lon':
            return lon
    return dispatch
...
def extract_words(text):
    ...
    for elements in text:
        if elements not in ascii_letters: # If object is not an ASCII
            text = text.replace(elements, ' ') # Replace unwanted object with blank
...
def analyze_tweet_sentiment(tweet):
    ...
    text_in_tweet = tweet_words(tweet)
    for word in text_in_tweet:
        if has_sentiment(get_word_sentiment(word)):
            total_sentiment_value += sentiment_value(get_word_sentiment(word))
            counter += 1
    if counter !=0:#can write if counter == 0: / average = make_sentiment else: its the line below (but this new way is condensed)
...
def find_centroid(polygon):
    ...
    for vertex in range(0, len(polygon) - 1): #can change all vertex to i
        constant_formula = ((latitude(polygon[vertex]) * longitude(polygon[vertex + 1])) - (latitude(polygon[vertex + 1]) * longitude(polygon[vertex])))
        # Constant formula in centroid_x, centroid_y, and area
        centroid_x += ((latitude(polygon[vertex]) + latitude(polygon[vertex + 1])) * constant_formula)
        centroid_y += ((longitude(polygon[vertex]) + longitude(polygon[vertex + 1])) * constant_formula)
...
def find_state_center(polygons):
    ...
    -156.21763
    """
    new_centroid_x, new_centroid_y, new_area = 0, 0, 0
    for polygon in polygons:
        centroid_x, centroid_y, area = find_centroid(polygon)
        new_centroid_x += (centroid_x * area)
        new_centroid_y += (centroid_y * area)
        new_area += (area)
    average_latitude = ((new_centroid_x) / (new_area))
    average_longtitude = ((new_centroid_y) / (new_area))
    return make_position(average_latitude, average_longtitude)

    centroid_x, centroid_y, area = 0, 0, 0 #starting here other way of writing this
    for i in polygons:
...
def group_tweets_by_state(tweets):
    ...
    for tweet in tweets:
        ...
        for state in us_centers: # Finds closest state from tweet
            distance = geo_distance(tweet_location(tweet), us_centers[state])
    ...
    tweets_by_state = {} #other way to write this starts here
    ...
    for state in tweets_by_state:
        ...
        for tweet in list_of_tweets:
            ...
            if has_sentiment(tweet_sentiment):
                total_sentiment += tweet_sentiment # Increase count of sentiment (or is it total_sentiment += sentiment_value(tweet_sentiment)
