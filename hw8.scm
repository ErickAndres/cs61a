...
(define (derive-sum exp var)  ;second and third element respectively addend and other thing 
  (+ (derive (addend exp) var) (derive (augend exp) var))
  (if (= addend 0)
    0  
    (if (= augend 0)
    0
    (make_sum (derive addend) (derive augend)))))
...
(define (derive-product exp var)
  ...
  ;;;0
   ;;; (if (= multiplicand 0)
   ;;; 0
  (make-sum (make-product (derive (multiplier exp) var) (multiplicand exp))
...
(define (make-exponentiation base exponent)
  (if (and (number? base) (number? exponent)) (* base (make-exponent base (- exponent 1)))
    (cons '^ (cons base (cons exponent)))))
