...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('unequal formal parameters and arguments')
        else:
            i = 0
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        defined_env = env.define(target, scheme_eval(vals.second.first, env))
        return target
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        defined_env = env.define(target.first, do_lambda_form(Pair(target.second, vals.second), env))
        return target.first
    else:
...
def do_and_form(vals, env):
    ...
    for i in range(len(vals) - 1):
        sub_exp = scheme_eval(vals[i], env)
        if sub_exp == False:
            return False
...
def do_or_form(vals, env):
    ...
    for i in range(len(vals) - 1):
        sub_exp = scheme_eval(vals[i], env)
        if scheme_true(sub_exp):
            return quote(sub_exp)
    return vals[len(vals) - 1]
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):   
        raise SchemeError('not well-formed list of symbols ' + str(formals))    
    else:
        copy_formals = []
        for el in formals:
            if not scheme_symbolp(el):
                raise SchemeError('list contains non-symbols')
            elif el in copy_formals:
                raise SchemeError('there are repeated symbols')
            copy_formals.append(el)
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
        	...
        	#solution here
            expr, env = do_let_form(rest, env) #what are you doing here? 
        else:
        	#solution here
